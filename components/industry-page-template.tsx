...
export default function IndustryPageTemplate({ path, industry, overview, benefits, faqs, relatedLinks = [] }: IndustryPageProps) {
  ...
  return (
    <main>
      ...
      <section className="w-full py-20 md:py-28 px-6 md:px-8 bg-white"><Reveal className="max-w-4xl mx-auto space-y-6"><><div className="space-y-2"><span className="text-sm font-semibold tracking-wide uppercase text-accent">The workflow</span><h2 className="text-4xl md:text-5xl font-bold text-primary text-balance">What tends to make {industry.name} accounting different</h2></div><p className="text-lg text-muted leading-8">{overview}</p></></Reveal></section>

      <section className="w-full py-20 md:py-28 px-6 md:px-8 bg-input"><div className="max-w-5xl mx-auto"><Reveal className="text-center space-y-4 mb-14"><><span className="text-sm font-semibold tracking-wide uppercase text-accent">Where support can fit</span><h2 className="text-4xl md:text-5xl font-bold text-primary text-balance">The parts of the workflow that can be delegated</h2><p className="max-w-2xl mx-auto text-muted leading-7">The right scope depends on your systems, team and review process. These are practical areas where additional capacity can be useful without taking control away from your team.</p></></Reveal><ul className="grid grid-cols-1 md:grid-cols-2 gap-6">{benefits.map((benefit, i) => <Reveal key={i}><li className="h-full flex items-start gap-4 p-6 bg-white rounded-2xl border border-border/70 shadow-[0_1px_3px_rgba(15,23,42,0.04)]"><Check className="shrink-0 text-accent w-5 h-5 mt-0.5" aria-hidden="true" /><p className="text-foreground leading-7">{benefit}</p></li></Reveal>)}</ul></div></section>

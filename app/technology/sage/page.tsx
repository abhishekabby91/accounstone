...
export default function SagePage() {
  ...
  return (
    <main>
      ...
      <section className="w-full py-8 md:py-12 px-6 md:px-8 bg-white"><div className="max-w-5xl mx-auto"><Reveal className="space-y-6"><><span className="text-sm font-semibold tracking-wide uppercase text-accent">What buyers usually care about</span><h2 className="text-2xl md:text-3xl font-bold text-primary text-balance">Sage is the system. Someone still has to keep every entity reconciled.</h2><p className="text-lg text-muted leading-8">A Sage file can be fully set up and still fall behind if reconciliations, categorization and month-end review aren't handled consistently — and that gets harder with multiple companies or locations to consolidate. We work inside your existing Sage setup on that recurring accounting routine, while implementation and system configuration stay with your Sage partner or internal IT team.</p></></Reveal></div></section>

      <section className="w-full py-8 md:py-12 px-6 md:px-8 bg-input"><div className="max-w-5xl mx-auto"><Reveal className="text-center space-y-4 mb-14"><><span className="text-sm font-semibold tracking-wide uppercase text-accent">Workflows</span><h2 className="text-2xl md:text-3xl font-bold text-primary text-balance">The Accounting Work Around Sage</h2></></Reveal><ul className="grid grid-cols-2 md:grid-cols-2 gap-3 md:gap-6">{workflows.map((item, i) => <Reveal key={i}><li className="flex items-start gap-2 sm:gap-4 p-3.5 sm:p-6 bg-white rounded-2xl border border-border/70"><Check className="w-4 h-4 sm:w-5 sm:h-5 text-accent shrink-0 mt-0.5 sm:mt-0" aria-hidden="true" /><span className="text-sm sm:text-base leading-5 sm:leading-7">{item}</span></li></Reveal>)}</ul></div></section>

      <section className="w-full py-8 md:py-12 px-6 md:px-8 bg-white"><div className="max-w-5xl mx-auto"><Reveal className="text-center space-y-4 mb-10"><><span className="text-sm font-semibold tracking-wide uppercase text-accent">Common starting points</span><h2 className="text-xl md:text-2xl font-bold text-primary">The software may not be the problem</h2></></Reveal><div className="grid grid-cols-1 md:grid-cols-3 gap-6"><div className="p-7 rounded-2xl bg-input border border-border/70"><h3 className="font-bold text-primary text-xl mb-3">Cleanup</h3><p className="text-muted leading-7">Reconciliations are behind, categories have drifted or old transactions need review before a repeatable routine can start.</p></div><div className="p-7 rounded-2xl bg-input border border-border/70"><h3 className="font-bold text-primary text-xl mb-3">Consolidation</h3><p className="text-muted leading-7">Multi-company or multi-location structures need intercompany reconciliations handled consistently before group reporting can be trusted.</p></div><div className="p-7 rounded-2xl bg-input border border-border/70"><h3 className="font-bold text-primary text-xl mb-3">Capacity</h3><p className="text-muted leading-7">Your team knows Sage, but recurring bookkeeping keeps competing with higher-value work each close cycle.</p></div></div></div></section>

      <section className="w-full py-8 md:py-12 px-6 md:px-8 bg-input"><div className="max-w-5xl mx-auto text-center"><span className="text-sm font-semibold uppercase tracking-wide text-accent">Related</span><h2 className="text-xl md:text-2xl font-bold text-primary mt-3 mb-8">Connect Sage to the accounting work</h2><div className="flex flex-wrap justify-center gap-3"><Link href="/services/bookkeeping" className="px-4 py-2 rounded-lg bg-white text-primary font-medium hover:bg-border transition-colors">Bookkeeping</Link><Link href="/services/accounting" className="px-4 py-2 rounded-lg bg-white text-primary font-medium hover:bg-border transition-colors">Accounting Services</Link><Link href="/services/accounts-payable" className="px-4 py-2 rounded-lg bg-white text-primary font-medium hover:bg-border transition-colors">Accounts Payable</Link></div></div></section>

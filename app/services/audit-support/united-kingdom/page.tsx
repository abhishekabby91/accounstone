...
export default function AuditSupportUKPage() {
  return <main>
    ...
    <section className="w-full py-8 md:py-12 px-6 md:px-8 bg-white"><div className="max-w-5xl mx-auto rounded-2xl bg-primary text-white p-8 md:p-10"><span className="text-sm font-semibold uppercase tracking-wide text-white/70">The line we do not cross</span><h2 className="text-3xl font-bold mt-3 mb-6">Preparation moves. Assurance does not.</h2><p className="text-white/85 leading-7 mb-6">Everything above is preparation and documentation work performed for the company, not audit work performed for the auditor. The engaged registered auditor directs the audit, evaluates the evidence, and signs the opinion.</p><ul className="grid md:grid-cols-2 gap-4">{retained.map((item, i) => <li key={i} className="flex items-start gap-3"><Check className="w-5 h-5 text-white shrink-0 mt-0.5" aria-hidden="true" /><span className="text-white/85 leading-6">{item}</span></li>)}</ul></div></section>

    <FAQSection subtitle="Questions" items={faqs} columns={2} />

    <section className="w-full py-12 md:py-16 px-6 md:px-8 bg-input"><div className="max-w-5xl mx-auto text-center"><h2 className="text-3xl font-bold text-primary mb-5">Keep the UK service cluster connected</h2>
